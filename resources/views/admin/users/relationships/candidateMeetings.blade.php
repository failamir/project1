...
<div class="card">
    ...
    <div class="card-body">
        <div class="table-responsive">
            <table class=" table table-bordered table-striped table-hover datatable datatable-candidateMeetings">
                <thead>
                    <tr>
                        <th width="10">
                ...
                <tbody>
                    @foreach($meetings as $key => $meeting)
                        <tr data-entry-id="{{ $meeting->id }}">
                            ...
                            <td>
                                {{ $meeting->job->date_expired ?? '' }}
                            </td>
                            <td>
                                {{ $meeting->candidate->name ?? '' }}
                            </td>
                            <td>
                                {{ $meeting->candidate->phone_number ?? '' }}
                            </td>
                            <td>
...
@endcan
  ...
  let table = $('.datatable-candidateMeetings:not(.ajaxTable)').DataTable({ buttons: dtButtons })
