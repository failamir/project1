...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.registrationFlow.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.registration-flows.update", [$registrationFlow->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('job') ? 'is-invalid' : '' }}" name="job_id" id="job_id">
                    @foreach($jobs as $id => $entry)
                        <option value="{{ $id }}" {{ (old('job_id') ? old('job_id') : $registrationFlow->job->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('job'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('flow') ? 'is-invalid' : '' }}" type="text" name="flow" id="flow" value="{{ old('flow', $registrationFlow->flow) }}">
                @if($errors->has('flow'))
                    <div class="invalid-feedback">
                        {{ $errors->first('flow') }}
                    </div>
